...
bool Maps::SortBlockEvents(df::map_block *block,
    ...
    vector <df::block_square_event_designation_priorityst *> *designation_priorities)
{
    if (veins)
        veins->clear();
    if (ices)
    ...
    if (items)
        items->clear();
    if (designation_priorities)
        designation_priorities->clear();
    ...
    for (size_t i = 0; i < block->block_events.size(); i++)
    {
        ...
        case block_square_event_type::designation_priority:
            if (designation_priorities)
                designation_priorities->push_back((df::block_square_event_designation_priorityst *)evt);
            break;
        }
    }
    return true;
